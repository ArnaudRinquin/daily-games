import { getLinkForUser, getProfileByPublicIdentifier, link, unlink } from '../db/linkedin';
...
export function registerLinkedIn(bot: AppBot): void {
  bot.chatType('private').command('linkedin', async (ctx) => {
    ...
    if (arg === 'off') {
      const removed = await unlink(ctx.env.DB, userId);
    ...
    if (!profile) {
      await ctx.reply(
        `No linkedin.com/in/${slug} on the leaderboard yet. ` +
          'You need to be connected to the captain on LinkedIn and to have finished at least one game today; ' +
